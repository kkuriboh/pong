...
int main(void) {
  ...
  Vector2 ballPosition = {(float)screenWidth / 2 - 5,
  ...
  while (!WindowShouldClose()) {
    ...
    if (!pause) {
      if (IsKeyDown(KEY_UP))
        player2.playerPosition.y -= 10.0f;
      if (IsKeyDown(KEY_DOWN))
        player2.playerPosition.y += 10.0f;
      if (IsKeyDown(KEY_W))
        player1.playerPosition.y -= 10.0f;
      if (IsKeyDown(KEY_S))
        player1.playerPosition.y += 10.0f;

      if (player1.playerPosition.y < 0) {
        player1.playerPosition.y = 0;
      }
      if (player1.playerPosition.y > screenHeight - 60) {
        player1.playerPosition.y = screenHeight - 60;
      }
      if (player2.playerPosition.y < 0) {
        player2.playerPosition.y = 0;
      }
      if (player2.playerPosition.y > screenHeight - 60) {
        player2.playerPosition.y = screenHeight - 60;
      }
      ...
      if (ballPosition.x >= (screenWidth - ballRadius)) {
        player1.score += 1;
        ballPosition = {(float)screenWidth / 2 - 5,
                        (float)screenHeight / 2 - 5};
